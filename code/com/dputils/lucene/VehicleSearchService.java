...
public class VehicleSearchService
{
	...
	public void init()
	{
		...
		String path = webroot + File.separatorChar + ".." + File.separatorChar + "lucene" + File.separatorChar
				+ "carinsu";
	...
	public VehicleSearchDto search(VehicleSearchVo searchVo)
	{
		...
		try
		{
			...
			result = new VehicleSearchDto(vehicles, getKeywords("", searchVo.getKey()), vehicles.size(),
	...
	private Query buildQuery(VehicleSearchVo searchVo) throws ParseException, IOException
	{
		...
		{
			rootQuery.add(new PrefixQuery(new Term(FAMILY_NAME, searchVo.getFamilyName())), BooleanClause.Occur.MUST);
		}
		if (StringUtils.isNotBlank(searchVo.getBrandName()))
		{
			rootQuery.add(new PrefixQuery(new Term(BRAND_NAME, searchVo.getBrandName())), BooleanClause.Occur.MUST);
		}
		if (StringUtils.isNotBlank(searchVo.getEngineDesc()))
		{
			rootQuery.add(new PrefixQuery(new Term(ENGINEDESC, searchVo.getEngineDesc())), BooleanClause.Occur.MUST);
		}
		if (StringUtils.isNotBlank(searchVo.getGearboxName()))
		{
			rootQuery.add(new PrefixQuery(new Term(GEARBOX_NAME, searchVo.getGearboxName())), BooleanClause.Occur.MUST);
		}
		return rootQuery;
	}
	...
	private String getExactKey(String key)
	{
		...
		for (int i = 0; i < key.length() - 1; i++)
		{
			...
			if ((!behand.matches("^[\u4E00-\u9FFF]+$") && !after.matches("^[\u4E00-\u9FFF]+$"))
					|| (!behand.matches("^[\u4E00-\u9FFF]+$") && sign))
			{
				result += behand;
				sign = true;
			}
	...
	public void batchBuildIndex(List<Vehicle> vehicles)
	{
		...
		{
			LoggerUtil.error("[vehicle search error]error happens when update lucene index:" + e.getMessage(), e);
		}
		finally
		{
			if (indexWriter == null)
			...
			{
				LoggerUtil.error("[vehicle search error]error happens when close index writer:" + e.getMessage(), e);
			}
		}
	}
	...
	private void openIndexSearcher()
	{
		...
		{
			LoggerUtil.error("[vehicle earch error]error happens when openIndexSearcher:" + e.getMessage(), e);
		}
	}
	...
	private Document getDocument(Vehicle vehicle)
	{
		...
		Field brandNameField = new Field(BRAND_NAME, vehicle.getBrandName(), Field.Store.YES, Field.Index.NOT_ANALYZED);
		//brandNameField.setBoost(2F);
		document.add(brandNameField);
		document.add(new Field(FAMILY_NAME, vehicle.getFamilyName(), Field.Store.YES, Field.Index.NOT_ANALYZED));
		document.add(new Field(VERSION_NAME, vehicle.getVersionName(), Field.Store.YES, Field.Index.NOT_ANALYZED));
		document.add(new Field(STANDARD_NAME, vehicle.getStandardName(), Field.Store.YES, Field.Index.NOT_ANALYZED));
		document.add(new Field(PRICE, String.valueOf(vehicle.getPrice()), Field.Store.YES, Field.Index.NO));
		document.add(new Field(ENGINEDESC, vehicle.getEngineDesc(), Field.Store.YES, Field.Index.NOT_ANALYZED));
		document.add(
				new Field(GEARBOX_NAME, vehicle.getGearBoxStringName(), Field.Store.YES, Field.Index.NOT_ANALYZED));
		...
		String brandAndFamily = vehicle.getBrandName() + vehicle.getFamilyName()
				+ stringSeparate(vehicle.getBrandName()) + stringSeparate(vehicle.getFamilyName());
	...
	public static class VehicleSearchVo implements Cloneable
	{
		...
		{
			return "VehicleSearchVo{" + "key='" + key + '\'' + ", id='" + id + '\'' + ", page=" + page + ", pageSize="
					+ pageSize + ", brandName='" + brandName + '\'' + ", standardName='" + standardName + '\''
					+ ", familyName='" + familyName + '\'' + ", engineDesc='" + engineDesc + '\'' + ", gearboxName='"
					+ gearboxName + '\'' + ", versionName='" + versionName + '\'' + ", seatCount='" + seatCount + '\''
					+ ", price=" + price + '}';
		}
	}
}
