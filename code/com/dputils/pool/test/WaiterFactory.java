...
import com.dputils.pool.object.DefaultPooledObject;
...
public class WaiterFactory<K> implements PooledObjectFactory<Waiter>, KeyedPooledObjectFactory<K, Waiter>
{
	...
	public WaiterFactory(long activateLatency, long destroyLatency, long makeLatency, long passivateLatency,
		...
		synchronized (this)
		{
			if (activeCount >= maxActive)
			{
				throw new IllegalStateException(
						"Too many active instances: " + activeCount + " in circulation with maxActive = " + maxActive);
			}
			else
			{
				activeCount++;
	...
	public PooledObject<Waiter> makeObject(K key) throws Exception
	{
		synchronized (this)
		{
			...
			else
			{
				...
				{
					throw new IllegalStateException("Too many active " + "instances for key = " + key + ": "
							+ count.intValue() + " in circulation " + "with maxActivePerKey = " + maxActivePerKey);
				}
				else
				{
					activeCounts.put(key, Integer.valueOf(count.intValue() + 1));
